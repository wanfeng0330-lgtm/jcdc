...
const mainFlow: FlowNode[] = [
  { icon: Download, title: '内容输入', desc: '图片/视频/音频/文本', color: 'from-cyan-500 to-blue-500' },
  { icon: Search, title: 'OCR识别', desc: '文字提取与结构化', color: 'from-blue-500 to-indigo-500' },
  { icon: Mic, title: 'Whisper语音', desc: '语音转文字分析', color: 'from-indigo-500 to-purple-500' },
  { icon: Brain, title: 'AI风险引擎', desc: '多维度特征分析', color: 'from-purple-500 to-pink-500' },
  { icon: Drama, title: '情绪分析', desc: '情绪操控模式识别', color: 'from-pink-500 to-rose-500' },
  { icon: RefreshCw, title: '传播分析', desc: '传播路径与影响建模', color: 'from-rose-500 to-orange-500' },
  { icon: BarChart2, title: '可信度评分', desc: '综合风险评级输出', color: 'from-orange-500 to-amber-500' },
  { icon: User, title: '人机协同', desc: '专家验证与反馈', color: 'from-amber-500 to-emerald-500' },
];
...
const coreValues = [
  { icon: Search, title: '信息可信度', desc: '多维度评估信息可靠性' },
  { icon: Globe, title: '风险传播', desc: '追踪虚假信息传播路径' },
  { icon: Brain, title: '认知偏差', desc: '识别利用认知漏洞的攻击' },
  { icon: Drama, title: '情绪操控', desc: '检测情绪操控与诱导模式' },
  { icon: BookOpen, title: '媒介素养', desc: '构建信息风险认知防线' },
  { icon: Shield, title: 'AI内容治理', desc: '建立AIGC内容治理体系' },
];

const collabSteps = [
  { icon: Bot, title: 'AI检测', desc: '高速筛选与初判' },
  { icon: User, title: '专家验证', desc: '深度判断与定性' },
  { icon: RefreshCw, title: '反馈迭代', desc: '模型持续优化' },
];
  ...
  return (
    <PageContainer>
            ...
            {mainFlow.map((node, i) => {
              ...
              return (
                <div key={i}>
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-br ${node.color} flex items-center justify-center shrink-0 shadow-lg`}
                    >
                      <IconComp size={18} className="text-white" strokeWidth={1.5} />
                    </div>
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center shrink-0">
                    <ModIcon size={20} className="text-accent" strokeWidth={1.5} />
                  </div>
                  <div className="flex-1 min-w-0">
      ...
      <Section delay={0.6} className="mb-6">
        <div className="glass-card p-4">
          <h3 className="text-sm font-semibold mb-4 inline-flex items-center gap-2 gradient-text">
            <Handshake size={14} className="text-accent" strokeWidth={1.5} />
            人机协同结构
          </h3>
                ...
                return (
                  <div key={i} className="flex-1">
                    <div className="glass-card-sm p-3 text-center">
                      <StepIcon size={20} className="mx-auto mb-1 text-accent" strokeWidth={1.5} />
                      <div className="text-xs font-medium">{step.title}</div>
                      <div className="text-[10px] text-muted">{step.desc}</div>
                    </div>
                    {i < collabSteps.length - 1 && (
                      <div className="text-accent text-xs text-center mt-2">⇌</div>
                    )}
                  </div>
                );
              })}
          ...
          <div className="grid grid-cols-2 gap-3">
            {coreValues.map((item, i) => {
              ...
              return (
                ...
                >
                  <ItemIcon size={20} className="mx-auto mb-1 text-accent" strokeWidth={1.5} />
